...
SEG_CHANNELS = 2
SEG_NET_SIZE = (128, 128, SEG_CHANNELS)
...
class Environment:
    def __init__ (self, raw, lbl, SEG_checkpoints_paths):
    ...
    def setup_nets (self, SEG_checkponts_paths):
        ...
        for path in SEG_checkponts_paths:
            net = FCN2D (SEG_CHANNELS, 1).to (self.device)
    ...
    def refine (self):
        ...
        with torch.no_grad():
            ...
            raw_patch = resize (raw_patch, SEG_NET_SIZE [:2], order=0, mode='reflect', preserve_range=True)
            raw_patch = np.expand_dims (np.expand_dims (raw_patch, 0), 0) # (1, 1, H, W)
            #append old mask
            raw_patch = np.repeat (raw_patch, 2, 1) #black out for current situation # (1, 2, H, W)
    ...
    def observation (self):
        ...
        raw_patch = resize (raw_patch, SEG_NET_SIZE [:2], order=0, mode='reflect', preserve_range=True).astype (np.uint8)
        mask_patch = resize (mask_patch, SEG_NET_SIZE [:2], order=0, mode='reflect', preserve_range=True).astype (np.uint8)
    ...
    def get_gt_label_RGB (self):
        ...
        gt_lbl = self.lbl [self.state.img_id, st[0]: st[0]+sz[0], 
                            st[1]: st[1] + sz[1]]
        gt_lbl = resize (gt_lbl, SEG_NET_SIZE [:2], order=0, mode='reflect', preserve_range=True).astype (np.uint8)
        ret = gt_lbl.flatten ().tolist ()
        ret = list (map (index2rgb, ret))
        ret = np.array (ret, dtype=np.uint8)
        ret = ret.reshape (SEG_NET_SIZE [:2] + (3,))
        return ret
    ...
    def get_labeled_mask_RGB (self):
        ...
        labeled_mask = resize (labeled_mask, SEG_NET_SIZE [:2], order=0, mode='reflect', preserve_range=True).astype (np.uint8)
