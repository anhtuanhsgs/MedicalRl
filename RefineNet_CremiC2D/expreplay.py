...
class ExpReplay(DataFlow, Callback):
    ...
    def _populate_exp(self):
        ...
        current_level = self.player.state.node.level
        obs_from_step, reward, isOver, info = self.player.step(act)
        if (info['down_level']):
            self.stack += [ { 'old_obs': self._current_ob,
                         'old_score': info ['current_score'],
                         'old_act': act,
                        }]
            self._current_ob = obs_from_step
            return
        # global CNT
        # CNT += 1
        # # #DEBUG
        # if CNT > 200:
        #     print ("\n\nNEW SAMPLE TO MEMORY")
        #     debug_img = []
        #     for debug_obs in [old_s, obs_from_step]:
        #         debug_raw = debug_obs [...,0]
        #         debug_mask = debug_obs [...,1]
        #         debug_refined_mask = debug_obs [...,2]
        #         debug_zoomed_mask = debug_obs [...,3]
        #         debug_img += [np.concatenate ([debug_raw, debug_mask, debug_refined_mask, debug_zoomed_mask], 1)]
        #     debug_img = np.concatenate (debug_img, 0)
        #     print ('action:', act, 'reward', reward, 'isOver', isOver)
        ...
        self._current_game_scores[current_level].feed (reward)
        self._current_ob = self.player.observation ()
        new_level = self.player.state.node.level
        if (info['up_level']):
            old_info = self.stack.pop ()
            old_score = old_info ['old_score']
            old_obs = old_info ['old_obs']
            old_act = old_info ['old_act']
            new_score = self.player.cal_metric ()
            delayed_reward = new_score - old_score
            isOver=False
            self._current_game_scores[new_level].feed (delayed_reward)
            self.mem.append(Experience(old_obs, old_act, delayed_reward, isOver))
            # #DEBUG
            # if CNT > 200:
            #     print ("\n\nNEW DELAYED SAMPLE TO MEMORY")
            #     debug_img = []
            #     for debug_obs in [old_obs, self._current_ob]:
            #         debug_raw = debug_obs [...,0]
            #         debug_mask = debug_obs [...,1]
            #         debug_refined_mask = debug_obs [...,2]
            #         debug_zoomed_mask = debug_obs [...,3]
            #         debug_img += [np.concatenate ([debug_raw, debug_mask, debug_refined_mask, debug_zoomed_mask], 1)]
            #     debug_img = np.concatenate (debug_img, 0)
            #     print ('delayed_action:', old_act, 'delayed_reward', delayed_reward, 'delayed_isOver', isOver)
        ...
        if isOver:
            for l in range (NUM_LEVELS):
                self._current_game_scores[l].feed (0)
            if info['ale.lives'] == 0:  # only record score when a whole game is over (not when an episode is over)
                for l in range (NUM_LEVELS):
                    self._player_scores[l].feed(self._current_game_scores[l].sum)
                    self._current_game_scores[l].reset()
            self._current_ob = self.player.reset()
            self.stack = []
    ...
    def __iter__(self):
        ...
        while True:
            # print len (self.mem), self.history_len, self.batch_size
