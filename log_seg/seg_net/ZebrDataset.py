...
class ZebrDataset (Dataset):
    ...
    def __getitem__ (self, idx):
        ...
        z_crop = [z[0] - self.rng.randint (z_range // 4, z_range//4*3), 
                                z[1] + self.rng.randint (z_range // 4, z_range//4*3)]
        y_crop = [y[0] - self.rng.randint (y_range //4, y_range//4*3), 
                                y[1] + self.rng.randint (y_range // 4, y_range//4*3)]
        x_crop = [x[0] - self.rng.randint (x_range // 4, x_range//4*3), 
                                x[1] + self.rng.randint (x_range // 4, x_range//4*3)]
