...
def rand_score (gt_lbl, pred_lbl):
    return adjusted_rand_score (pred_lbl.flatten (), gt_lbl.flatten ())
...
class EM_env (gym.Env):
    ...
    def step (self, action):
        ...
        patches [2][::] = new_prob
        new_label = label (self.prob > self.cell_thres, background=0)
        if self.type == 'train':
            new_score = self.metric (self.gt_lbl, new_label)
            reward = (new_score - self.old_score) * 100
