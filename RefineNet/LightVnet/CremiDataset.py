...
class CremiDatasetRefine (Dataset):
    ...
    def __getitem__ (self, idx):
        ...
        if use_ulv_mask:
            ...
            raw_patch_ulv_ds = np.expand_dims (raw_patch_ulv_ds, 0)
            addition_mask = np.zeros_like (raw_patch_ulv_ds)
            raw_patch_ulv_ds = np.concatenate ([raw_patch_ulv_ds, addition_mask], 0)
            raw_patch_ulv_t = torch.tensor (raw_patch_ulv_ds, device=self.device, dtype=torch.float32, requires_grad=False) / 255.0
            raw_patch_ulv_t = raw_patch_ulv_t.unsqueeze (0)
...
class CremiDatasetSquare (Dataset):
    def __init__ (self, set_type, X, y, level, ds_factor, zoom_factor):
        self.raw_list = X
        self.lbl_list = y
        self.sample_size = list (ORIGIN_SIZE)
        self.square_size = [64, 128, 256, 512]
        for i in range (level):
            self.sample_size [0] = math.ceil (self.sample_size [0] * zoom_factor)
            self.sample_size [1] = math.ceil (self.sample_size [1] * zoom_factor) 
        self.out_size = (math.ceil (self.sample_size[0] * ds_factor), 
                        math.ceil (self.sample_size[1] * ds_factor))
        for sq_size in self.square_size:
            if sq_size >= self.out_size [0]:
                self.out_size = (sq_size, sq_size)
                break
...
class CremiDatasetRefineSquare (Dataset):
    def __init__ (self, set_type, X, y, level, ds_factor, zoom_factor, uplevel_model, ds_factor_ulv, device):
        self.raw_list = X
        self.lbl_list = y
        self.sample_size = list (ORIGIN_SIZE)
        ...
        self.out_size = (math.ceil (self.sample_size[0] * ds_factor), 
                        math.ceil (self.sample_size[1] * ds_factor))
        self.out_size = self.squaredSize (self.out_size)

        self.ulv_in_size = (math.ceil (self.ulv_size [0] * ds_factor_ulv),
                            math.ceil (self.ulv_size [1] * ds_factor_ulv))
        self.ulv_in_size = self.squaredSize (self.ulv_in_size)
    ...
    def squaredSize (self, size):
        sqSize = [64, 128, 256, 512]
        for s in sqSize:
            if s >= size[0]:
                return (s, s)
    ...
    def __getitem__ (self, idx):
        ...
        #Use only prediction, no raw
        if self.rng.rand () > 0.5 and use_ulv_mask:
            raw_patch = np.zeros_like (raw_patch)
