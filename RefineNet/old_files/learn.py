...
def dqn_learning(env,
    ...
    Q = q_func(in_channels, num_actions).type(dtype).to(dqn_device)
    Q_target = q_func(in_channels, num_actions).type(dtype).to(dqn_device)
    ...
    for t in itertools.count():
        ...
        if t < learning_starts:
            ...
            if sample > threshold:
                obs = torch.tensor(observations, device=dqn_device, dtype=torch.float) / 255.0
                with torch.no_grad ():
        ...
        if t % LOG_SAMPLE_EVERY_N_STEPS == 0 and t >= LOG_SAMPLE_EVERY_N_STEPS:
            ...
            while not done:
                print (obs.shape)
                obs_t = torch.tensor(obs.transpose (3, 0, 1, 2), device=dqn_device, dtype=torch.float).unsqueeze(0) / 255.0
                with torch.no_grad ():
            ...
            log_vol = np.concatenate ([raw, mask], 1)
            io.imsave ('sample_log/log_vol' + str(t + 1) + '.tif', log_vol)
