...
class Residual_Conv (nn.Module):
	def __init__ (self, in_ch, out_ch):
		...
		self.batch_norm_i = nn.InstanceNorm2d (in_ch) 
		self.conv1_i = nn.Sequential (nn.Conv2d (in_ch, out_ch, 3, bias=False, padding=1), nn.ReLU ())

		self.conv2_i = nn.Sequential (nn.Conv2d (in_ch, out_ch // 2, 3, bias=False, padding=1), nn.ReLU ())
		self.conv2_m = nn.Sequential (nn.Conv2d (out_ch // 2, out_ch, 3, bias=False, padding=1), nn.ReLU ())
		self.conv2_o = nn.Sequential (nn.ConvTranspose2d (out_ch, out_ch, 1, padding=0, bias=False), nn.ReLU ())

		self.conv1_o = nn.Sequential (nn.Conv2d (out_ch, out_ch, 3, bias=False, padding=1), nn.ReLU ())
		self.instance_norm_o = nn.InstanceNorm2d (out_ch)
...
class Up (nn.Module):
	def __init__ (self, in_ch, out_ch):
		...
		self.up_module = nn.Sequential (
			...
			nn.ReLU (),
			Residual_Conv (out_ch, out_ch),
			nn.ConvTranspose2d (out_ch, out_ch, 1, padding=0, bias=False),
			nn.ReLU (),
			nn.InstanceNorm2d (out_ch)
		)
...
class Unet (nn.Module):
	...
	def __init__ (self, in_ch, features, out_ch):
		...
		self.last_layer = nn.Sequential (
			nn.Conv2d (features[0], out_ch, 3, padding=1),
			nn.Sigmoid ()
		)
