...
def train_refine (train_data, test_data, n_epoc, loss_func, optimizer, lr_scheduler, i_iter=0):
    if args.refine:
        loggers = []
        print ("log dirs:")
        for level in range (4):
...
if __name__ == '__main__':
    ...
    if args.refine is not None:
        ...
        isbi2012 = ISBI2012_Dataset16 ('train', X_train, y_train, level, ZOOM_FACTOR)
        isbi2012_test = ISBI2012_Dataset16 ('test', X_test, y_test, level, ZOOM_FACTOR)
        model = FusionNet (IN_CH, FEATURES, OUT_CH).to (device)
