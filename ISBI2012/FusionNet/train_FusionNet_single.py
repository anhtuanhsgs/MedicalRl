...
REFINE_PATHS = [
    'checkpoints/0_0.4/checkpoint_389400.pth.tar',
]
...
def eval (test_data, loss_func, hasTarget=True):
    ...
    for i_ipoc in range (n_epoc):
        ...
        for i_batch, sample in enumerate (train_data):
            ...
            if i_batch == len (train_data) - 1 and (i_ipoc + 1) % LOG_PERIOD == 0:
                ...
                raw_t, pred_t = eval (test_data, loss_func, hasTarget=False)
...
def setup_global_var (args):
    global LOG_DIR, CHECKPOINT_SAVE_PATH, DS_FACTOR, ZOOM_FACTOR, BATCH_SIZE, device
...
if __name__ == '__main__':
    ...
    cremiData = ISBI2012_Dataset16 ('train', X_train, y_train, level, ZOOM_FACTOR)
    cremiData_test = ISBI2012_Dataset16 ('test', X_test, y_test, level, ZOOM_FACTOR)
